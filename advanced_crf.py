...
def makeTokens(it, winsize, step=1):
    it=iter(it)
...
def returnExtractedFeatures(utterances):
    ...
    for i in range(0 , len(utterances)):
        ...
        if (token_pos_list != None):
            ...
            #Make bi-gram token
            biGramToken = list(makeTokens(tokenList,2))
            for item in biGramToken:
                biResult = "/".join(item)
                tokenString="TOKEN_" + biResult
                utterLevelList.append(tokenString)
            #Make bi-gram pos
            bigramPos = list(makeTokens(posList, 2))
            for item in bigramPos:
                poResult = "/".join(item)
                posString = "POS_" + poResult
                utterLevelList.append(posString)

            triGramToken = list(makeTokens(tokenList, 3))
            for item in triGramToken:
                biResult = "/".join(item)
                tokenString = "TOKEN_" + biResult
                utterLevelList.append(tokenString)
            # Make bi-gram pos
            trigramPos = list(makeTokens(posList, 3))
            for item in trigramPos:
                poResult = "/".join(item)
                posString = "POS_" + poResult
                utterLevelList.append(posString)
        else:
...
testLabeledList = [getListOfLabel(a) for a in list(get_data(testDirectoryName))]
